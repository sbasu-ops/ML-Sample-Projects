...
def load_subset20_imagenet(path, dtype=np.float32, subtract_mean=True):
    """
    ...
    class_names = [wnid_to_words[wnid] for wnid in wnids if wnid in subset_wid]
...
def load_sub20_imagenet_corrupt(path, class_labels, mean_image, severity = '3', typeof = 'all',subtract_mean=True):
