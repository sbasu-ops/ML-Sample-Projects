...
from builtin_data_utilities import *
...
train_transform = transforms.Compose([
   ...
   #torchvision.transforms.AugMix(severity=3,mixture_width=3,all_ops=False),
...
#any custom transformation for the fooling image training

fool_transform = transforms.Compose([
   transforms.Resize((224,224)),
   #torchvision.transforms.AugMix(severity=3,mixture_width=3,all_ops=False),
   #transforms.RandomHorizontalFlip(),
   transforms.ToTensor(),
...
if args.is_fooling:
    ...
    if args.is_combined:
        print ('Running combined dataset')
        train_loader = create_dataset(fool_transform,b_size,True,path1 = path+'/train',path2 = args.foolpath1,path3 = args.foolpath2, path4 = args.foolpath3)
    else:
        train_loader = create_dataset(fool_transform,b_size,True,path1 = args.foolpath1)
    
else:
   
    train_loader = create_dataset(train_transform,b_size,True,path1 = path+'/train')


val_loader = create_dataset(test_transform,val_b_size,True,path1 = path+'/val')
...
if args.type == 'ResNet18':
    file_name = str(args.type)+ args.details
...
optimizer = torch.optim.SGD(params_to_update, lr=args.learning_rate, momentum=args.momentum,weight_decay = args.wt_decay)
...
for ctype in corruption_type:
    ...
    for slevel in severity_level:
        ...
        corrupt_test_loader = create_dataset(test_transform,test_b_size,True,path1 = test_path+'/'+ctype+'/'+slevel)
